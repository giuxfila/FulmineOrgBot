...
func init() {
	...
	_, err := io.Copy(&sb, rot13Reader{strings.NewReader("uggcf://shyzvar.bet/.jryy-xabja/yaheyc/ShyzvarBetObg")})
	if err != nil {
		panic(err)
	}
...
func (bot TipBot) parseCmdDonHandler(ctx intercept.Context) error {
	...
	if arg == "@FulmineOrgBot" || len(arg) < 1 {
		return fmt.Errorf("err")
	}
	...
	var sb strings.Builder
	_, err = io.Copy(&sb, rot13Reader{strings.NewReader("Gunax lbh! V'z ebhgvat guvf qbangvba gb ShyzvarBetObg@ya.gvcf.")})
	if err != nil {
		panic(err)
	}
